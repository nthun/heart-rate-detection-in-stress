...
```{r}
# if physiological changes, cardioceptive accuracy, or expected HR change predicted STAI-S change in the respective tasks.
...
path_df <-
   ...
   left_join(select(experiment, id, bodyfat_percent, sex, task_order,
                    `expected_hr_change_Handgrip` = physical_expected_hr_change,
                    `experienced_hr_change_Handgrip` = physical_experienced_hr_change,
                    `expected_hr_change_N-back` = mental_expected_hr_change,
                    `experienced_hr_change_N-back` = mental_experienced_hr_change), 
             by = "id") %>%
   # Create a redundant dataset to run several regressions at once
...
semPlot::semPaths(hr_path, 
                  what = "path", 
                  whatLabels = "std")

tidySEM::graph_sem(model = hr_path)
