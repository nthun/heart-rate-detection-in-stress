...
install.packages(c("tidyverse", "skimr", "lme4", "performance", "sjPlot", "here", "emo", "broom.mixed", "psych", "easystats", "ggbeeswarm", "naniar", "skmir"))
```
...
hypo3 <-
   ...
   mutate(`N-back_stai_react_ca` = map(data, 
          ...
          `N-back_stai_react_hrp` = map(data, 
                                     ~lm(`stai_react_N-back` ~ hrp + 
                                         mental_expected_hr_change + 
                                         task_order +
                                         value,
                                         data = .x)),
          Handgrip_stai_react_hrp = map(data, 
                                     ~lm(stai_react_Handgrip ~ hrp + 
                                         physical_expected_hr_change + 
                                         task_order + 
                                         value,
                                         data = .x))) %>% 
...
ggExtra::ggMarginal(hrp_hbp, type = "density", alpha = .6,
...
path_df <-
   ...
   select(id, starts_with("meanhr_"),
          ...
          `stai_react_N-back`) %>% 
   # Put HR reactivity metrics expected and experienced change together
   left_join(select(experiment, id, bodyfat_percent, sex, task_order,
             ...
             by = "id") %>%
   # Create a redundant dataset to run several regressions at once
   pivot_longer(cols = -c("id", "bodyfat_percent", "sex", "task_order"),
                names_to = c("metric","task"),
                names_pattern = "^(.*)_.*_(.*)",
                values_to = "value") %>%  
...
hr_model2 <- 
  "stai ~ expectedhr
   stai ~ experiencedhr
   stai ~ meanhr
   experiencedhr ~ expectedhr
   experiencedhr ~ meanhr
   meanhr ~ expectedhr
"

# Fit model in lavaan
hr_path2 <- sem(model = hr_model2, data = path_df, group = "task")

# tidySEM way of building and running sem
model <-
  tidy_sem(path_df) %>%
  add_paths(hr_model2)
...
# Return results
summary(hr_path2, standardized = TRUE, fit.measures = TRUE)

# Plotting SEM
# Format node labels
pretty_nodes <-
  get_nodes(hr_path2, label = "name") %>% 
...
graph_sem(hr_path2, 
          # Format edges (to std)
          edges = get_edges(hr_path2, label = "est_sig_std") %>% 
                  filter(op != "~~"),
          nodes = pretty_nodes,
          layout = layout2)
...
graph_sem(model = hr_path, 
          edges = get_edges(hr_path, label = "est_sig_std") %>% 
                  filter(op != "~~"),
          nodes = pretty_nodes,
          layout = layout)
