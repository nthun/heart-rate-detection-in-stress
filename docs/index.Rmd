...
cb_ = (hr_XXszubj – hr_XXobj) / hr_XXobj 
...
phys_trans %>% 
   select(id, marker, task_order, sex, bmi, 
...
skin_avg_mod <-
   phys_trans %>% 
   select(id, task_order, marker, sex, bmi, all_of(skin_avg)) %>% 
...
skin_increase_mod <-
   phys_trans %>% 
   select(id, marker, task_order, sex, bmi, all_of(skin_increase)) %>% 
...
The reaction to stress tasks is moderated by cardioceptive accuracy.
...
cardioceptive_models <-
   phys_trans %>% 
   select(id, 
          marker, meanhr, lg_hf_percent, lg_eda_avg, 
          ca, cb, hrp) %>% 
...
phys_trans %>% 
   ...
   labs(title = "Association between cardioceptive accuracy and mean HR",
        subtitle = "The correlation is similar in all tasks",
        y = "Mean HR (bpm)",
        x = "Cardioceptive accuracy")
...
phys_trans %>% 
   ...
   facet_wrap(~marker) +
   geom_smooth(method = "lm") +   
   labs(title = "Association between cardioceptive bias and mean HR",
        subtitle = "The correlation is similar in all tasks",
        y = "Mean HR (bpm)",
        x = "Cardioceptive bias")
...
There is a positive correlation between cardioceptive accuracy, caridoceptive bias, and heart rate perception.

```{r}

correlation(phys_trans, select = c("ca", "cb", "hrp"),
            p_adjust = "fdr") %>% 
   print_html()
