...
phys_trans %>% 
   ...
   scale_color_viridis_d() +
      labs(title = "Association between cardioceptive metrics and mean HR",
        ...
        color = "Task",
        y = "Mean HR (bpm) / lgRMSSD (lgms)",
        x = "Cardioceptive accuracy / Heart rate perception") +
...
hypo2 <-
   ...
   pivot_longer(-c(id, ca, hrp, bodyfat_percent, sex, `stai_react_N-back`, stai_react_Handgrip, task_order),
   ...
   mutate(`ca_N-back_mod` = map(data, 
                            ~lm(value ~ `stai_react_N-back` * ca +
                                        bodyfat_percent + sex + task_order, 
                                data = .x)),
          `hrp_N-back_mod` = map(data, 
                            ~lm(value ~ `stai_react_N-back` * hrp +
                                        bodyfat_percent + sex + task_order, 
                                data = .x)),
          ca_Handgrip_mod = map(data, 
                            ~lm(value ~ stai_react_Handgrip * ca +
                                         bodyfat_percent + sex + task_order, 
                                data = .x)),
          hrp_Handgrip_mod = map(data, 
                            ~lm(value ~ stai_react_Handgrip * hrp +
                                        bodyfat_percent + sex + task_order, 
                                data = .x))
   ) %>% 
...
phys_trans %>% 
   select(id, marker, task_order, sex, bodyfat_percent, 
          meanhr, lg_hf_percent, lg_eda_avg) %>% 
