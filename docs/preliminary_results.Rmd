---
title: "Heart rate detection and stress - Preliminary result document"
author: "Tamas Nagy"
date: "`r Sys.Date()`"
output: 
  html_document:
...
To access the latest version, with cognitive tasks merged:
https://nthun.github.io/heart-rate-detection-in-stress/index.html 
...
We fitted linear regressions for each task (cognitive1, cognitive2, and physical) separately. We investigated if physiological changes, cardioceptive accuracy, or expected HR change predicted STAI-S change in the respective tasks. We used task order as a covariate. 

We found no associations between cardioceptive accuracy and STAI-S change.
Expected HR change did predict STAI-S change in all cognitive tasks.
...
hypo3 <-
   ...
   left_join(select(experiment, 
                    ...
                    by = "id") %>%
   pivot_longer(meanhr_react_cognitive1:eda.avg_react_physical,
                names_pattern = "(.*)_react_(.*)",
                names_to = c("metric", "task")) %>% 
   group_by(metric, task) %>% 
   # Scale all numeric vaiarables (by metric and task)
   mutate(across(c(stai_react_cognitive:ca,
                   physical_expected_hr_change:mental_expected_hr_change,
                   value), 
                 ~scale(.x) %>% 
                  as.numeric())) %>%    
   nest() %>% 
   mutate(cognitive_stai_react = map(data, 
                                     ~lm(stai_react_cognitive ~ ca + 
                                         mental_expected_hr_change + 
                                         task_order +
                                         value,
                                         data = .x)),
          physical_stai_react = map(data, 
                                     ~lm(stai_react_physical ~ ca + 
                                         physical_expected_hr_change + 
                                         task_order + 
                                         value,
                                         data = .x))) %>% 
   pivot_longer(cognitive_stai_react:physical_stai_react) %>% 
      filter((str_detect(task, "cognitive") & str_detect(name, "cognitive")) |
          (str_detect(task, "physical") & str_detect(name, "physical"))) %>% 
...
We fitted linear regressions for each task (cognitive1, cognitive2, and physical) separately. We investigated if actual (ECG measured) HR change in the respective tasks predicted subjective HR change, and if expectations predicted subjective HR change better. We included sex, body fat percent, and task order as covariates.
...
hypo4 <-
   ...
   mutate(physical_by_expected = map(data, 
                            ~lm(physical_experienced_hr_change ~ 
                                physical_expected_hr_change +
                                bodyfat_percent + sex + task_order, 
                                data = .x)),
          cognitive_by_expected = map(data, 
                            ~lm(mental_experienced_hr_change ~ 
                                mental_expected_hr_change +
                                bodyfat_percent + sex + task_order, 
                                data = .x)),
          physical_by_react = map(data, 
                            ~lm(physical_experienced_hr_change ~ 
                                hr_reactivity +
                                bodyfat_percent + sex + task_order, 
                                data = .x)),
          cognitive_by_react = map(data, 
                            ~lm(mental_experienced_hr_change ~ 
                                hr_reactivity +
                                bodyfat_percent + sex + task_order, 
                                data = .x))
   ...
   pivot_longer(physical_by_expected:cognitive_by_react) %>% 
   filter((str_detect(task, "cognitive") & str_detect(name, "cognitive")) |
          (str_detect(task, "physical") & str_detect(name, "physical"))) %>% 
   mutate(name = str_replace(name, "cognitive", task)) %>% 
   ungroup()
