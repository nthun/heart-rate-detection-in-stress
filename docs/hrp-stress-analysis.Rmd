---
title: "Heart rate detection and stress analysis"
author: "Tamas Nagy"
date: "2022-07-26"
output: 
  html_document:
...
```{r}
phys_trans <- 
  read_csv(here::here("data/processed/physiology_clean.csv"))

experiment <- 
  read_csv(here::here("data/processed/survey_clean.csv"))

reactivity <-
  read_csv(here::here("data/processed/reactivity_clean.csv"))
...
# Subjective response to stress: STAI-S
...
phys_trans %>% 
   select(id, marker, task_order, sex, bodyfat_percent, 
          all_of(skin_avg)) %>% 
   pivot_longer(c(all_of(skin_avg)), values_drop_na = TRUE) %>%
...
hr_model <- 
  "stai ~ expectedhr
   ...
   experiencedhr ~ task_order
   expectedhr ~ task_order
   meanhr ~ expectedhr
   meanhr ~ task_order + sex + bodyfat_percent
"
